...
router.get('/', async (req, res) => {
  ...
  query = query.order('logged_at', { ascending: false }).order('created_at', { ascending: false });
  if (limit) { const lim = parseInt(limit); const off = parseInt(offset || 0); query = query.range(off, off + lim - 1); }
  const { data, error } = await query;
  if (error) throw error;
  const result = (data || []).map(l => ({ ...l, exercise_name: l.exercises?.name, muscle_group: l.exercises?.muscle_group, category: l.exercises?.category, exercises: undefined }));
  res.json(result);
});
...
router.put('/:id', async (req, res) => {
  ...
  const { error: updErr } = await supabase.from('workout_logs').update(updates).eq('id', req.params.id);
  if (updErr) throw updErr;
  const { data: log, error } = await supabase.from('workout_logs').select('*, exercises(name, muscle_group)').eq('id', req.params.id).single();
  if (error) throw error;
  const result = { ...log, exercise_name: log.exercises?.name, muscle_group: log.exercises?.muscle_group, exercises: undefined };
  res.json(result);
});
...
router.post('/batch', async (req, res) => {
  ...
  const formatted = entries.map(e => ({
    exercise_id: e.exercise_id, weight: e.weight, reps: e.reps || 1, sets: 1,
    notes: e.notes || null, logged_at: date, user_id: req.user.id,
  }));
  const { data: logs, error } = await supabase.from('workout_logs').insert(formatted).select('*, exercises(name, muscle_group)');
  if (error) throw error;
  const result = (logs || []).map(l => ({ ...l, exercise_name: l.exercises?.name, muscle_group: l.exercises?.muscle_group, exercises: undefined }));
  res.status(201).json(result);
});
...
router.delete('/:id', async (req, res) => {
  ...
  if (checkErr) throw checkErr;
  if (!existing) return res.status(404).json({ error: 'Not found' });
  if (existing.user_id !== req.user.id && req.user.role !== 'superadmin') return res.status(403).json({ error: 'Not your log' });
  const { data, error } = await supabase.from('workout_logs').delete().eq('id', req.params.id).select();
  if (error) throw error;
