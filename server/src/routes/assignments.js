...
async function verifySameOrg(req, userId) {
  if (req.user.role === 'superadmin') {
    const { data: target } = await supabase.from('profiles').select('org_id').eq('id', userId).maybeSingle();
    if (!target || target.org_id !== req.user.org_id) return false;
  } else {
    const { data: profile } = await supabase.from('profiles').select('admin_id, org_id').eq('id', userId).maybeSingle();
    if (!profile || profile.org_id !== req.user.org_id || profile.admin_id !== req.user.id) return false;
  }
  return true;
}
...
router.get('/exercises/:clientId', async (req, res) => {
  ...
  if (!(await verifySameOrg(req, clientId))) {
...
router.post('/exercises', async (req, res) => {
  ...
  if (!(await verifySameOrg(req, client_id))) {
    return res.status(403).json({ error: 'Not your client' });
  }
...
router.delete('/exercises/:id', async (req, res) => {
  ...
  if (!(await verifySameOrg(req, existing.client_id))) {
    return res.status(403).json({ error: 'Not your client' });
  }
